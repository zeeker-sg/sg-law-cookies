...
def run_source(
    ...
    news pipeline. The watermark advances only after a successful store."""
    ...
    if item_type_for(table) != "news":
        raise PipelineError(
            f"{source} routes to the judgment pipeline, which is not built yet (PRD phase 3)"
        )
    ...
    if not dry_run and not any(
        e.zeeker_db == zeeker_db and e.table == table for e in db.list_registry(conn)
    ):
